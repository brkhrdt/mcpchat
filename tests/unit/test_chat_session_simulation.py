...
from mcp_simple_chatbot.core.chat_session import ChatSession, UserInput, ToolResult, LLMResponse, ToolCall
...
async def test_simulated_conversation(mock_chat_session):
    ...
    conversation_script = [
        {
            ...
            "expected_tool_call": {"tool": "get_weather", "args": {"location": "London"}},
            ...
            "expected_messages_after_turn_1": [
                ...
                {"role": "assistant", "content": MagicMock()} # LLM's thinking/tool call (content will be parsed)
            ],
            "expected_messages_after_turn_2": [
                {"role": "system", "content": MagicMock()}, # Initial system message
                {"role": "user", "content": "What's the weather like in London?"},
                {"role": "assistant", "content": MagicMock()}, # LLM's thinking/tool call
                {"role": "system", "content": '{"temperature": "15°C", "condition": "partly cloudy"}'}, # Tool result
                {"role": "assistant", "content": "The weather in London is currently 15°C and partly cloudy."}
            ]
        },
        {
            "input_type": "user",
    ...
    assert "You are a helpful assistant with access to these tools" in chat_session.messages[0]["content"]
    ...
    for i, step in enumerate(conversation_script):
        ...
        if step["input_type"] == "user":
            ...
            if "expected_tool_call" in step:
                ...
                tool_id_pattern = r"tool_\d+" # We don't know the exact ID, just the pattern
                # Find the tool call in the messages to get its ID
                tool_call_message = next(
                    (msg for msg in chat_session.messages if "commentary to=functions" in msg.get("content", "")),
                    None
                )
                assert tool_call_message is not None, "Tool call message not found in history"
                ...
                await chat_session.input_queue.put(
                    ToolResult(dummy_tool_id, step["tool_result"], step["expected_tool_call"]["tool"])
                )
            ...
            expected_messages = step.get("expected_messages_after_turn_2" if "expected_tool_call" in step else "expected_messages_after_turn_1")
            ...
            for j, expected_msg in enumerate(expected_messages):
                ...
                if expected_msg["role"] == "system":
                    ...
                    if j == 0: # Initial system message
                        ...
                        assert json.loads(actual_msg["content"]) == json.loads(expected_msg["content"])
                else:
