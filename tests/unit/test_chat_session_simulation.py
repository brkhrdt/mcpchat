...
async def mock_chat_session():
    ...
    mock_server.execute_tool = AsyncMock(return_value={"temperature": "15°C", "condition": "partly cloudy"})
...
async def test_simulated_conversation(mock_chat_session):
    ...
    llm_response_map = {
        ...
        None,  # Tool calls don't add message content directly
    ...
    expected_messages = [
        {"role": "system", "content": "You are a helpful assistant"},  # Will check contains
        {"role": "user", "content": "What's the weather like in London?"},
        {"role": "system", "content": '{"temperature": "15°C", "condition": "partly cloudy"}'},
    ...
    await chat_session._process_conversation_turn()  # Processes user input, calls LLM, executes tool
    
    # Simulate tool result
    await chat_session.input_queue.put(ToolResult("tool_1", {"temperature": "15°C", "condition": "partly cloudy"}, "get_weather"))
    await chat_session._process_conversation_turn()  # Processes tool result, calls LLM for final response
    ...
    mock_server.execute_tool.assert_called_once_with("get_weather", {"location": "London"})
